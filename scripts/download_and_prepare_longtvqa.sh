...
# Hugging Face downloads can be flaky on lab servers. Keep these defaults
# conservative; all can be overridden by exporting them before running.
HF_DOWNLOAD_MAX_WORKERS="${HF_DOWNLOAD_MAX_WORKERS:-1}"
HF_DOWNLOAD_RETRIES="${HF_DOWNLOAD_RETRIES:-5}"
HF_DOWNLOAD_RETRY_SLEEP="${HF_DOWNLOAD_RETRY_SLEEP:-20}"
...
echo "HF max workers: $HF_DOWNLOAD_MAX_WORKERS"
echo "HF retries: $HF_DOWNLOAD_RETRIES"
...
download_dataset_repo() {
  ...
  for attempt in $(seq 1 "$HF_DOWNLOAD_RETRIES"); do
    echo "Download dataset repo: $repo_id -> $local_dir (attempt $attempt/$HF_DOWNLOAD_RETRIES)"
    if "$DL_BIN" download "$repo_id" \
      --repo-type dataset \
      --local-dir "$local_dir" \
      --max-workers "$HF_DOWNLOAD_MAX_WORKERS"; then
      return 0
    fi

    echo "[WARN] Download failed: $repo_id (attempt $attempt/$HF_DOWNLOAD_RETRIES)" >&2
    if [[ "$attempt" -lt "$HF_DOWNLOAD_RETRIES" ]]; then
      echo "Retry after ${HF_DOWNLOAD_RETRY_SLEEP}s..."
      sleep "$HF_DOWNLOAD_RETRY_SLEEP"
    fi
  done

  echo "[ERROR] Failed to download dataset repo after retries: $repo_id" >&2
  return 1
}

if [[ "$DL_BIN" == "hf" ]]; then
  download_dataset_repo "$LONGTVQA_REPO" "$LONGTVQA_DIR"
  download_dataset_repo "$LONGTVQA_PLUS_REPO" "$LONGTVQA_PLUS_DIR"
else
  download_dataset_repo "$LONGTVQA_REPO" "$LONGTVQA_DIR"
  download_dataset_repo "$LONGTVQA_PLUS_REPO" "$LONGTVQA_PLUS_DIR"
fi
